...
- [x] Inter-annotator protocol + demo κ (`eval_inter_annotator.py`, `data/gold/`)
- [x] Automated summary (`evaluation_suite.py` → `docs/evaluation/evaluation_results_summary.md`)

## Before Friday (priority order)

### Must-do (report credibility)

1. **Dual annotation (real)** — Patrick and Darren independently fill `data/gold/annotator_b.json` from `statements.json` **without** looking at `annotator_a.json`. Run `python evaluation/eval_inter_annotator.py`. Replace demo disagreements in report.
2. **Blind Track 3 rater** — One classmate completes `python evaluation/track3_evaluation.py --sheet`. Report agreement with team gold.
3. **Update report** — Use `docs/evaluation/evaluation_results_summary.md` numbers; reframe abstract (developmental validation, not clinical trial).
...
python evaluation/evaluation_suite.py          # full summary markdown
...
python evaluation/eval_inter_annotator.py --export-template
python evaluation/eval_inter_annotator.py
python evaluation/track3_evaluation.py
python evaluation/extraction_evaluation.py     # needs ANTHROPIC_API_KEY
```
