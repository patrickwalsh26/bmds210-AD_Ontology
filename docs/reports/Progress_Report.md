...
> **Note (revised May 29, 2026):** This Markdown file is an early mirror; the authoritative, up-to-date report is [Progress_Report.tex](Progress_Report.tex). The framing below has been updated to match it following expert review by Dr. Magnus (see [Magnus_Expert_Review_2026-05-27.md](Magnus_Expert_Review_2026-05-27.md)).

Patients document end-of-life (EOL) care preferences through several instruments: advance health care directives (AHCDs), POLST forms, and clinician-entered code-status orders. Contrary to a common assumption, the difficulty is not primarily that these are unstructured free text — the better AHCDs (the California statutory form, the University of California directive) are largely **checkbox** instruments. The difficulty is that they are **low-resolution and disease-blind**: checkbox forms collapse conditional, graded preferences into coarse options and omit the device- and disease-specific decisions that dominate advanced heart failure (ICD deactivation, LVAD management, inotrope escalation). And at the bedside in an acute crisis, it is **POLST and code-status orders — not the AHCD — that clinicians act on**. An estimated 67% of ICU patients lack decision-making capacity at EOL, yet fewer than 37% of US adults have any directive on file, and those that exist are rarely interoperable with EHR systems.

This project builds an OWL ontology of EOL care preferences — the Advance Directive Ontology (ADO) — that enables a reasoner to match a patient's documented preferences to their current clinical situation and infer the implied care decision. ADO is a **decision-support and documentation aid** — concretely, a tool to help pre-populate an advance-care-planning progress note — and is **not** a legal advance directive: its output carries no statutory force and does not replace an AHCD, POLST, or code-status order. We scope the ontology to **advanced heart failure (HF)**, specifically five common clinical decision points where these preferences are consulted:
